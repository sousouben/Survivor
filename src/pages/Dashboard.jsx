...
const Dashboard = () => {
  const { isDarkMode, isEnergySaveMode, toggleDarkMode, toggleEnergySaveMode } = useTheme();
  ...
  const quickActions = [
    ...
    // {  // Section Préparation retirée
    //   title: 'Préparation',
    //   description: 'Checklist avant départ',
    //   icon: PreparationIcon, // This was causing the error
    //   path: '/preparation',
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        >
          <Battery className={`mx-auto mb-2 ${isCharging ? 'text-survival-green dark:text-survival-green-light' : 'text-muted-foreground'}`} size={24} />
          <p className="text-xs text-muted-foreground">Batterie</p>
          <p className="text-sm font-bold text-foreground">{batteryLevel !== null ? `${batteryLevel}%` : 'N/A'}</p>
        </motion.div>
        ...
        >
          ...
          <p className="text-sm font-bold text-foreground">{isDarkMode ? 'Nuit' : 'Jour'}</p>
        </motion.button>
        ...
        <motion.button
          ...
          className="glass-effect rounded-lg p-4 text-center"
        >
          <Zap className={`mx-auto mb-2 ${isEnergySaveMode ? 'text-orange-400' : 'text-muted-foreground'}`} size={24} />
          <p className="text-xs text-muted-foreground">Économie</p>
          <p className="text-sm font-bold text-foreground">{isEnergySaveMode ? 'ON' : 'OFF'}</p>
        </motion.button>
      </div>
            ...
            >
              <Link to={action.path}>
                <div className="glass-effect rounded-xl p-6 h-36 flex flex-col justify-between hover:bg-card/80 dark:hover:bg-card/70 transition-all">
                  <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${action.color} flex items-center justify-center mb-3 shadow-md`}>
      ...
      <motion.div
        ...
        className="mt-8 glass-effect rounded-lg p-4"
      >
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-foreground">Météo Locale</h3>
            <p className="text-muted-foreground text-sm">Conditions favorables</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-foreground">18°C</p>
            <p className="text-muted-foreground text-sm">Nuageux</p>
          </div>
        </div>
      </motion.div> 
      */}
