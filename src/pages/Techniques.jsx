...
import { ChevronRight, Droplets, Flame, Home as HomeIcon, Apple, Compass as CompassIcon, Shield, Image as ImageIcon } from 'lucide-react';
  ...
  const techniques = [
    {
      ...
      techniques: [
        {
          name: 'Ébullition',
          description: 'Faire bouillir l\'eau pendant 3-5 minutes pour éliminer les bactéries et virus.',
          steps: [
  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        <h1 className="text-3xl font-extrabold text-foreground mb-2 tracking-tight">Techniques de Survie</h1>
        <p className="text-muted-foreground text-lg">Guides pratiques pour toutes les situations</p>
      </div>
            ...
            >
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <h3 className="font-semibold text-lg text-foreground text-left">{category.title}</h3>
                </div>
                <motion.div
              ...
              >
                <div className="space-y-5">
                  {category.techniques.map((technique, techIndex) => (
                    ...
                    >
                      <h4 className="font-semibold text-md text-survival-green dark:text-survival-green-light mb-2">{technique.name}</h4>
                      <p className="text-muted-foreground text-sm mb-3">{technique.description}</p>
                      
                      {technique.image && (
                        <div className="my-3 rounded-md overflow-hidden">
                           <img-replace src={technique.image} alt={technique.name} className="w-full h-auto object-cover" />
                        </div>
                      )}

                      <div className="space-y-2">
                        <h5 className="font-medium text-survival-green dark:text-survival-green-light text-sm">Étapes :</h5>
                        <ol className="space-y-1.5 list-decimal list-inside">
                          {technique.steps.map((step, stepIndex) => (
                            <li key={stepIndex} className="text-muted-foreground text-sm flex items-start">
                              <span className="text-survival-green dark:text-survival-green-light mr-2 font-semibold">{stepIndex + 1}.</span>
                              <span>{step}</span>
                            </li>
                          ))}
                        </ol>
