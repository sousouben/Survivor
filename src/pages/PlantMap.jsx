  ...
  const ediblePlants = [
    {
      ...
      edible: true,
      medicinal: true,
      description: 'Feuilles comestibles crues ou cuites. Propriétés anti-inflammatoires.',
    ...
    },
    {
      ...
      region: 'france',
      position: [46.0000, 2.0000],
      edible: true,
      medicinal: true,
      description: 'Baies noires sucrées, excellentes crues ou en confiture. Feuilles en tisane.',
      season: 'Fin d\'été à automne',
      identification: 'Ronces épineuses, feuilles dentées, fleurs blanches ou roses.',
    ...
    },
    {
      ...
      region: 'france',
      position: [47.5000, 3.5000],
      edible: true,
      medicinal: false,
      description: 'Fruits à coque très nutritifs, riches en lipides et protéines.',
      season: 'Automne',
      identification: 'Arbuste, feuilles ovales dentées, fruits dans une cupule foliacée.',
    ...
    },
    {
      ...
      region: 'france',
      position: [45.0000, 1.0000],
      edible: true,
      medicinal: true,
      description: 'Petites fraises très parfumées. Feuilles et racines médicinales.',
      season: 'Printemps à été',
      identification: 'Plante rampante, feuilles trifoliées dentées, fleurs blanches.',
    ...
    },
    {
      ...
      region: 'france',
      position: [46.8000, 5.5000],
      edible: true,
      medicinal: true,
      description: 'Forte odeur d\'ail. Toutes les parties sont comestibles.',
      season: 'Printemps',
      identification: 'Feuilles larges et lancéolées, fleurs blanches en ombelle. Attention à ne pas confondre avec le Muguet (toxique).',
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        <h1 className="text-2xl font-bold text-white mb-2">Carte des Plantes</h1>
        <p className="text-gray-400">Plantes comestibles et dangereuses par région</p>
      </div>
      ...
      {/* Map */}
      <div className="mb-6 glass-effect rounded-lg overflow-hidden" style={{ height: '300px' }}>
        <MapContainer
          center={regions.find(r => r.id === selectedRegion)?.center || userLocation}
          zoom={6}
          ...
          return (
            <motion.div
              ...
              className={`glass-effect rounded-lg p-4 ${
                plant.edible ? 'border-l-4 border-green-500' : 'border-l-4 border-red-500'
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <DangerIcon className={getDangerColor(plant.danger)} size={16} />
                  </div>
                  <p className="text-gray-400 text-sm italic mb-2">{plant.scientificName}</p>
                  <p className="text-gray-300 text-sm mb-2">{plant.description}</p>
                  
                  {plant.edible && (
                    <div className="space-y-1 text-sm">
                      {plant.season && (
                        <p><span className="text-green-400">Saison:</span> {plant.season}</p>
                      )}
                      {plant.identification && (
                        <p><span className="text-blue-400">Identification:</span> {plant.identification}</p>
                      )}
                      {plant.preparation && (
                        <p><span className="text-yellow-400">Préparation:</span> {plant.preparation}</p>
                      )}
                    </div>
                  )}
                  
                  {!plant.edible && plant.identification && (
                    <p className="text-sm"><span className="text-red-400">Identification:</span> {plant.identification}</p>
                  )}
                </div>
      ...
      {filteredPlants.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="text-gray-400">Aucune plante trouvée pour cette recherche</p>
        </div>
      )}
    </motion.div>
  );
